...
export function FocusAreaEditDialog({
  focusAreaName,
  segId,
  startTime,
  endTime,
  ...
  const [selectedFocus, setSelectedFocus] = useState<string>(
    focusAreaNamesAndIds?.find((f) => f.name === focusAreaName)?.name ?? ""
  );
  ...
  useEffect(() => {
    if (
      focusAreaNamesAndIds.length &&
      !focusAreaNamesAndIds.map((f) => f.name).includes(selectedFocus)
    ) {
      setSelectedFocus(focusAreaNamesAndIds[0]?.name ?? "");
    }
  }, [focusAreaNamesAndIds, selectedFocus]);

  const updateMutation = useMutation({
    mutationFn: async (payload: { segment: Partial<TimerSegment> }) => {
      console.log("PAYLOAD", payload);
      await axios.post(`/api/segments/edit?segmentId=${segId}`, payload);
    },
  ...
  const handleSubmit = () => {
    ...
    updateMutation.mutate({
      segment: {
        ...
        focusAreaId: focusAreaNamesAndIds.find((f) => f.name === selectedFocus)
          ?.id,
      }
    });
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="p-0">
        ...
        <div className="px-4 pt-4 pb-2 flex gap-3 overflow-x-auto">
          {focusAreaNamesAndIds.map((f) => (
            <button
              key={f.id}
              onClick={() => setSelectedFocus(f.name)}
              className={`px-3 py-1.5 rounded-full border text-sm ${
                selectedFocus === f.name
